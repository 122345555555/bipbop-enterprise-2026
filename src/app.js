...
async function refresh(){
  try{
    ...
    if(!cfg.url||!cfg.key){ BBRender.renderAll(); if(window.BBFba) await BBFba.load(); return; }
    ...
    BBRender.renderAll();
    if(window.BBFba) await BBFba.load();
  }catch(e){
    state.errors.push(String(e.message||e));
    BBRender.renderAll();
  }
...
document.addEventListener("DOMContentLoaded",()=>{bind();if(window.BBFba) BBFba.init();BBRender.renderAll();refresh();});
})();
