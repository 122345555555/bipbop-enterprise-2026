window.BBRender = {
  ...
  renderAll(){
    ...
    BBUtils.el("dataHealth").innerHTML=BBAnalytics.reportDefs.map(r=>{
      ...
      return '<div class="action"><b>'+r[1]+'</b><br>'+(ok?'<span class="status-ok">✓ importato</span> — '+fc+' file, '+s.counts[r[0]]+' righe':'<span class="'+(r[2]==="obbligatorio"?'status-missing':'status-warn')+'">'+(r[2]==="obbligatorio"?'mancante obbligatorio':'mancante')+'</span>')+'</div>';
    }).join("");
    ...
    BBUtils.el("statusGrid").innerHTML=BBAnalytics.reportDefs.map(r=>{
      ...
      return '<div class="import-box '+(ok?'ok':'missing')+'"><h4>'+r[1]+'</h4><p><span class="pill">'+r[2]+'</span></p><b>Stato:</b> '+(ok?'🟢 Attivo':'⚪ Nessun file')+'<br><b>File attivi:</b> '+fc+'<br><b>Righe attive:</b> '+(s.counts[r[0]]||0)+'<br><b>Ultimo file:</b> '+(latest?.file_name||'—')+'</div>';
    }).join("");

    BBUtils.el("archiveTable").innerHTML=s.files.length?'<table><tr><th>Report</th><th>File</th><th>Righe</th><th>Colonne</th><th>Duplicato</th><th>Hash</th><th>Importato</th></tr>'+s.files.map(f=>'<tr><td><span class="pill">'+BBAnalytics.label(f.report_type)+'</span></td><td>'+f.file_name+'</td><td>'+f.row_count+'</td><td>'+f.column_count+'</td><td>'+(f.is_duplicate?'<span class="pill red">sì, non sommato</span>':'<span class="pill green">no</span>')+'</td><td class="small">'+String(f.fingerprint||"").slice(0,12)+'...</td><td>'+new Date(f.imported_at).toLocaleString("it-IT")+'</td></tr>').join("")+'</table>':'<div class="action">Nessun report importato.</div>';
    ...
    BBUtils.el("adsFilesBox").innerHTML=adFiles.length?'<h3>File Ads attivi</h3><table><tr><th>Tipo</th><th>File</th><th>Righe</th></tr>'+adFiles.map(f=>'<tr><td>'+BBAnalytics.label(f.report_type)+'</td><td>'+f.file_name+'</td><td>'+f.row_count+'</td></tr>').join("")+'</table>':'';

    const ar=BBAnalytics.asinRows(s.samples);
    if(window.BBFba) window.BBFba.setProducts(ar);
    BBUtils.el("asinBox").innerHTML=ar.length?'<table><tr><th>ASIN</th><th>Titolo</th><th>Vendite</th><th>Unità</th><th>Sessioni</th><th>Conv.</th></tr>'+ar.map(r=>'<tr><td>'+r.asin+'</td><td>'+r.title+'</td><td>'+BBUtils.euro(r.sales)+'</td><td>'+r.units+'</td><td>'+r.sessions+'</td><td>'+(r.cr?r.cr+"%":"—")+'</td></tr>').join("")+'</table>':'<div class="action">Importa Business Report o Transazioni.</div>';
