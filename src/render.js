window.BBRender = {
  ...
  renderAll(){
    ...
    if(fbaEl){
      ...
      const fbaItems=(rules.fbaItems||[]).map(item=>window.BBFbaManager?BBFbaManager.normalizeItem(item):item);
      ...
      const fbaResolved=r=>{
        const sug=BBAnalytics.fbaSuggestionForAsin ? BBAnalytics.fbaSuggestionForAsin(scopedSamples,r.asin) : null;
        const sale=BBUtils.num(r.salePrice ?? r.unitCost) || BBUtils.num(sug?.salePrice);
        const production=BBUtils.num(r.productionCost) || BBUtils.num(sug?.productionCost);
        const actual=BBAnalytics.fbaTestForItem ? BBAnalytics.fbaTestForItem(scopedSamples,r) : {units:0,sales:0,orders:0,remaining:BBUtils.num(r.quantitySent||r.qty),sellThrough:NaN};
        const recon=BBAnalytics.fbaReconciliationForItem ? BBAnalytics.fbaReconciliationForItem(scopedSamples,{...r,title:r.title || sug?.title || ""}) : {...actual,adSpend:0,adSales:0,notAttributedSales:actual.sales||0,tacos:NaN,acos:NaN,label:"Dati non disponibili",confidence:"bassa"};
        return {...r,title:r.title || sug?.title || "",salePrice:sale,unitCost:sale,productionCost:production,_suggestion:sug,_actual:actual,_recon:recon};
      };
      const resolvedFbaItems=fbaItems.map(fbaResolved);
      if(window.BBFbaManager) BBFbaManager.pruneSelection(resolvedFbaItems.map(r=>r.id));
      const fbaFilter=window.BBFbaManager?BBFbaManager.getFilter():"";
      const visibleFbaItems=fbaFilter?resolvedFbaItems.filter(r=>r.status===fbaFilter):resolvedFbaItems;
      const selectedFba=window.BBFbaManager?BBFbaManager.getSelected():new Set();
      const fbaSalePrice=r=>BBUtils.num(r.salePrice ?? r.unitCost);
      const fbaProductionCost=r=>BBUtils.num(r.productionCost);
      const productionTotal=resolvedFbaItems.reduce((a,r)=>a+(BBUtils.num(r.qty)*fbaProductionCost(r)),0);
      const inboundTotal=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r.inboundCost),0);
      const amazonShippingTotal=resolvedFbaItems.reduce((a,r)=>a+(BBUtils.num(r.qty)*BBUtils.num(r.amazonShipCost)),0);
      const revenueTotal=resolvedFbaItems.reduce((a,r)=>a+(BBUtils.num(r.qty)*fbaSalePrice(r)),0);
      const totalFbaCost=productionTotal+inboundTotal+amazonShippingTotal;
      const netFba=revenueTotal-totalFbaCost;
      const qty=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r.qty),0);
      const actualUnits=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._actual?.units),0);
      const actualSales=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._actual?.sales),0);
      const actualOrders=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._actual?.orders),0);
      const sentQty=resolvedFbaItems.reduce((a,r)=>a+(BBUtils.num(r.quantitySent)||BBUtils.num(r.qty)),0);
      const actualSellThrough=sentQty?actualUnits/sentQty*100:NaN;
      const fbaAdSpend=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._recon?.adSpend),0);
      const fbaAdSales=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._recon?.adSales),0);
      const fbaNotAttributed=resolvedFbaItems.reduce((a,r)=>a+BBUtils.num(r._recon?.notAttributedSales),0);
      const fbaTacos=actualSales?fbaAdSpend/actualSales*100:NaN;
      const fbaAcos=fbaAdSales?fbaAdSpend/fbaAdSales*100:NaN;
      const active=resolvedFbaItems.filter(r=>!["chiuso","stop"].includes(String(r.status||""))).length;
      const statusLabel=s=>window.BBFbaManager?BBFbaManager.label(s):s;
      const statusClass=s=>window.BBFbaManager?BBFbaManager.statusClass(s):"";
      const managedStatuses=window.BBFbaManager?BBFbaManager.STATUSES:[["da_preparare","Da preparare"],["inviato","Inviato"],["ricevuto","Ricevuto da Amazon"]];
      const legacyStatuses=window.BBFbaManager?BBFbaManager.LEGACY_STATUSES:[];
      const statusOptions=(current,includeLegacy=true)=>managedStatuses.concat(includeLegacy?legacyStatuses:[]).map(([value,text])=>'<option value="'+h(value)+'" '+(current===value?'selected':'')+'>'+h(text)+'</option>').join("");
      const statusCounts=Object.fromEntries(managedStatuses.map(([status])=>[status,resolvedFbaItems.filter(r=>r.status===status).length]));
      const statusDate=r=>r.statusChangedAt||r.updatedAt||r.createdAt||"";
      const transitionDays=r=>{
        const transit=window.BBFbaManager?BBFbaManager.daysBetween(r.sentAt||r.sendDate,r.receivedAt):null;
        const activation=window.BBFbaManager?BBFbaManager.daysBetween(r.receivedAt,r.activeAt):null;
        const total=window.BBFbaManager?BBFbaManager.daysBetween(r.sentAt||r.sendDate,r.activeAt):null;
      ...
      const shipmentInfo=r=>r.sentAt||r.sendDate
        ? '<b>'+h(BBUtils.dateIT(r.sentAt||r.sendDate))+'</b><br><span class="small">'+h(r.carrier||"Corriere —")+' · '+h(r.tracking||"Tracking —")+'</span><br><span class="small">'+h(r.shipmentId||"Shipment ID —")+' · '+h(r.packages||0)+' colli · '+h(r.weightKg||0)+' kg · '+h(r.quantitySent||r.qty||0)+' pz</span>'
        : '—';
      const historyHtml=r=>{
        const rows=(r.statusHistory||[]).slice().reverse();
        return '<details class="fba-history"><summary>Storico</summary>'+(rows.length?'<div class="fba-history-list">'+rows.map(entry=>'<div><b>'+h(statusLabel(entry.toStatus))+'</b><br><span>'+h(BBUtils.dateTimeIT(entry.changedAt))+(entry.fromStatus?' · da '+h(statusLabel(entry.fromStatus)):'')+'</span></div>').join("")+'</div>':'<span class="small">Nessun passaggio registrato.</span>')+'</details>';
      };
      fbaEl.innerHTML='<div class="grid3">'+[
        ...
        ["Vendite FBA rilevate",actualUnits||"—"],
        ["Ordini FBA rilevati",actualOrders||"—"],
        ["Ricavi FBA reali",actualUnits?BBUtils.euro(actualSales):"—"],
        ["Sell-through test",actualUnits?BBUtils.pct(actualSellThrough):"—"],
        ["Spesa Ads ASIN FBA",fbaAdSpend?BBUtils.euro(fbaAdSpend):"—"],
        ["Vendite attribuite Ads",fbaAdSales?BBUtils.euro(fbaAdSales):"—"],
        ["Vendite non attribuite Ads",actualSales?BBUtils.euro(fbaNotAttributed):"—"],
        ["TACoS reale FBA",Number.isFinite(fbaTacos)?BBUtils.pct(fbaTacos):"—"],
        ["ACOS attribuito FBA",Number.isFinite(fbaAcos)?BBUtils.pct(fbaAcos):"—"],
        ["Test attivi",active||"—"],
        ["Quantità consigliata","10 pz / ASIN"]
      ].map(x=>'<div class="kpi"><small>'+h(x[0])+'</small><strong>'+h(x[1])+'</strong></div>').join("")+'</div>'+
      '<h3 class="fba-status-title">Stato operativo FBA</h3><div class="fba-status-kpis">'+managedStatuses.map(([status,text])=>'<div class="fba-status-kpi '+statusClass(status)+'"><small>'+h(text)+'</small><strong>'+h(statusCounts[status]||0)+'</strong></div>').join("")+'</div>'+
      ...
      '<h3>Aggiungi ASIN FBA</h3><div class="fba-form">'+
        ...
        '<div><label>Stato</label><select id="fbaStatus">'+statusOptions("da_preparare")+'</select></div>'+
      ...
      '</div>'+
      '<div class="fba-list-head"><h3>ASIN da gestire in FBA</h3><div class="fba-list-tools"><label>Filtra per stato <select id="fbaStatusFilter"><option value="">Tutti gli stati</option>'+managedStatuses.map(([value,text])=>'<option value="'+h(value)+'" '+(fbaFilter===value?'selected':'')+'>'+h(text)+'</option>').join("")+'</select></label><span id="fbaSelectedCount" class="small">'+(selectedFba.size?selectedFba.size+' ASIN selezionati':'Nessun ASIN selezionato')+'</span><button id="fbaBulkSentBtn" type="button" '+(selectedFba.size?'':'disabled')+'>Segna come inviati</button></div></div>'+
      (resolvedFbaItems.length?(visibleFbaItems.length?'<div class="fba-table-wrap"><table class="decision-table fba-management-table"><tr><th></th><th>Stato</th><th>ASIN / Prodotto</th><th>Inviati</th><th>Venduti FBA</th><th>Residui test</th><th>Sell-through</th><th>Ricavi reali</th><th>Prezzo vendita</th><th>Produzione/pezzo</th><th>Invio Amazon</th><th>Sped. Amazon/pezzo</th><th>Ricavo stimato</th><th>Costo totale stimato</th><th>Netto stimato</th><th>Dettagli spedizione</th><th>Tempi</th><th>Ultimo cambio</th><th>Note</th><th>Azione</th></tr>'+visibleFbaItems.map(r=>{ const q=BBUtils.num(r.qty); const sale=fbaSalePrice(r); const production=fbaProductionCost(r); const revenue=q*sale; const inv=(q*production)+BBUtils.num(r.inboundCost)+(q*BBUtils.num(r.amazonShipCost)); const net=revenue-inv; return '<tr><td><input class="fba-row-check" data-fba-id="'+h(r.id)+'" type="checkbox" '+(selectedFba.has(String(r.id))?'checked':'')+' aria-label="Seleziona '+h(r.asin)+'"></td><td><select class="fba-inline-status '+statusClass(r.status)+'" data-fba-id="'+h(r.id)+'" data-current-status="'+h(r.status||'da_preparare')+'">'+statusOptions(r.status)+'</select></td><td>'+this.asinCell(r.asin,r.title)+'</td><td>'+h(r.quantitySent||r.qty||0)+'</td><td><b>'+h(r._actual?.units||0)+'</b></td><td>'+h(r._actual?.remaining ?? (r.quantitySent||r.qty||0))+'</td><td>'+h(Number.isFinite(r._actual?.sellThrough)?BBUtils.pct(r._actual.sellThrough):"—")+'</td><td><b>'+h(BBUtils.euro(r._actual?.sales||0))+'</b></td><td>'+h(BBUtils.euro(sale))+(r._suggestion&&!BBUtils.num(r.salePrice)?'<br><span class="small">stimato dai report</span>':'')+'</td><td>'+h(BBUtils.euro(production))+(r._suggestion&&!BBUtils.num(r.productionCost)?'<br><span class="small">da costi prodotto</span>':'')+'</td><td>'+h(BBUtils.euro(r.inboundCost||0))+'</td><td>'+h(BBUtils.euro(r.amazonShipCost||0))+'</td><td>'+h(BBUtils.euro(revenue))+'</td><td><b>'+h(BBUtils.euro(inv))+'</b></td><td class="'+(net<0?'stock-bad':'status-ok')+'">'+h(BBUtils.euro(net))+'</td><td>'+shipmentInfo(r)+'</td><td class="small">'+transitionDays(r)+'</td><td>'+h(BBUtils.dateTimeIT(statusDate(r)))+'</td><td class="small">'+h(r.notes||"—")+'</td><td><button class="secondaryBtn editFbaBtn" data-fba-id="'+h(r.id)+'" type="button">Modifica</button><button class="secondaryBtn deleteFbaBtn" data-fba-id="'+h(r.id)+'" type="button">Elimina</button>'+historyHtml(r)+'</td></tr>'; }).join("")+'</table></div>':'<div class="action">Nessun ASIN nello stato selezionato.</div>'):'<div class="action">Nessun ASIN FBA inserito. Parti da 3-5 prodotti gia venduti, 10 pezzi ciascuno.</div>')+
      '<h3>Riconciliazione vendite Test FBA</h3>'+
      '<div class="action"><b>Come leggere i dati</b><br>Il Report ordini è la fonte delle vendite reali. I report pubblicitari indicano solo le vendite attribuite da Amazon. La differenza viene mostrata come <b>non attribuita nei report Ads</b>, non come vendita organica certa.</div>'+
      (resolvedFbaItems.length?'<div class="fba-table-wrap"><table class="decision-table"><tr><th>ASIN / Prodotto</th><th>Vendite reali</th><th>Ads attribuite</th><th>Non attribuite Ads</th><th>Spesa Ads</th><th>TACoS reale</th><th>ACOS Ads</th><th>Click</th><th>Impression</th><th>Store</th><th>Riconciliazione</th></tr>'+resolvedFbaItems.map(r=>{ const x=r._recon||{}; const store=x.storeMatches?(x.storeViews+' visite · '+x.storeOrders+' ordini · '+BBUtils.euro(x.storeSales)):'Nessun collegamento diretto'; const cls=x.status==='periodi_non_allineati'||x.status==='non_riconciliabile'?'stock-bad':(x.status==='attribuita_ads'?'status-ok':''); return '<tr><td>'+this.asinCell(r.asin,r.title)+'</td><td><b>'+h(BBUtils.euro(x.sales||0))+'</b><br><span class="small">'+h(x.units||0)+' pz · '+h(x.orders||0)+' ordini</span></td><td>'+h(BBUtils.euro(x.adSales||0))+'</td><td><b>'+h(BBUtils.euro(x.notAttributedSales||0))+'</b></td><td>'+h(BBUtils.euro(x.adSpend||0))+'</td><td>'+h(Number.isFinite(x.tacos)?BBUtils.pct(x.tacos):"—")+'</td><td>'+h(Number.isFinite(x.acos)?BBUtils.pct(x.acos):"—")+'</td><td>'+h(x.clicks||0)+'</td><td>'+h(x.impressions||0)+'</td><td class="small">'+h(store)+'</td><td class="'+cls+'"><b>'+h(x.label||"—")+'</b><br><span class="small">Affidabilità: '+h(x.confidence||"—")+(x.undatedRows?' · '+h(x.undatedRows)+' righe Ads senza data':'')+'</span></td></tr>'; }).join("")+'</table></div>':'')+
      ...
      (resolvedFbaItems.length?'<table class="compact-table"><tr><th>ASIN</th><th>Descrizione</th><th>Pezzi da stampare</th><th>Costo produzione/pezzo</th><th>Note</th></tr>'+resolvedFbaItems.map(r=>'<tr><td><b>'+h(r.asin||"—")+'</b></td><td>'+h(r.title||"—")+'</td><td><b>'+h(r.qty||0)+'</b></td><td>'+h(BBUtils.euro(fbaProductionCost(r)))+'</td><td>'+h(r.notes||"—")+'</td></tr>').join("")+'</table>':'<div class="action">Aggiungi ASIN FBA per creare la lista stampa.</div>')+'</div>'+
      '<h3>Candidati dai dati attuali</h3>'+
      (candidates.length?'<table class="decision-table"><tr><th>ASIN / Prodotto</th><th>Vendite</th><th>Unità</th><th>Profitto</th><th>Margine</th><th>Perché candidato</th><th></th></tr>'+candidates.map(r=>{ const sug=BBAnalytics.fbaSuggestionForAsin ? BBAnalytics.fbaSuggestionForAsin(scopedSamples,r.asin) : null; return '<tr><td>'+this.asinCell(r.asin,r.title)+'</td><td>'+h(BBUtils.euro(r.sales))+'</td><td>'+h(r.units||0)+'</td><td class="'+((r.profit||0)<0?'stock-bad':'')+'">'+h(BBUtils.euro(r.profit))+'</td><td>'+h(BBUtils.pct(r.margin))+'</td><td>'+h((r.units||0)>=5?"Ha gia vendite e unita: buono per test Prime/FBA.":"Ha venduto: valuta se leggero e con margine sufficiente.")+'</td><td><button class="secondaryBtn pickFbaCandidateBtn" data-asin="'+h(r.asin)+'" data-title="'+h(r.title||"")+'" data-sale-price="'+h(sug?.salePrice||0)+'" data-production-cost="'+h(sug?.productionCost||0)+'" type="button">Usa</button></td></tr>'; }).join("")+'</table>':'<div class="action">Carica ordini o Profit Report per vedere candidati automatici.</div>');
    }
